...
class PaymentOptionsTest extends TestCase {
 ...
 public function test_admin_configures_methods_and_client_announces_zelle_payment(): void {
  ...
  $intent=ClientPaymentIntent::query()->latest('id')->firstOrFail();$response->assertRedirect(route('portal.make-payment.create',['plan'=>$plan->id]));
 ...
 public function test_plan_balances_are_exposed_and_offline_notice_can_be_cancelled_with_a_note(): void {
  ...
  $intent=ClientPaymentIntent::query()->where('payment_plan_id',$plan->id)->oldest('id')->firstOrFail();
  $this->assertSame('announced',$intent->status);
  $this->assertSame($data['client_note'],$intent->client_note);
  $this->get(route('portal.make-payment.create'))->assertOk()->assertSee('Admin notified of $50.00 Zelle payment.')->assertSee('data-cancel-payment-notification',false)->assertDontSee('plan.disabled=value',false);
