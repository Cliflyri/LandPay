...
@if($pendingPaymentIntents->isNotEmpty())<div class="admin-next-card mt-4"><h2>Payment notices awaiting admin confirmation</h2>@foreach($pendingPaymentIntents as $pending)<p class="mb-2">
<a href="{{ route('portal.make-payment.show', $pending) }}">
    {{ \App\Support\Money::format($pending->amount) }}
    by {{ str($pending->method)->replace('_', ' ')->title() }}
