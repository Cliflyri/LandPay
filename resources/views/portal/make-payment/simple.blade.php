...
if(squareLandPay){if(window.Square){try{const payments=Square.payments(squareConfig.application_id,squareConfig.location_id);payments.card().then(async card=>{squareCard=card;await squareCard.attach('#square-card-container')}).catch(()=>alert('Secure card entry could not be loaded. Please refresh the page.'))}catch{setTimeout(()=>alert('Secure card entry could not be loaded. Please refresh the page.'),0)}}else{setTimeout(()=>alert('Secure card entry could not be loaded. Please refresh the page.'),0)}}
