...
if(squareLandPay){if(window.Square){Square.payments(squareConfig.application_id,squareConfig.location_id).then(async payments=>{squareCard=await payments.card();await squareCard.attach('#square-card-container')}).catch(()=>alert('Secure card entry could not be loaded. Please refresh the page.'))}else{setTimeout(()=>alert('Secure card entry could not be loaded. Please refresh the page.'),0)}}
