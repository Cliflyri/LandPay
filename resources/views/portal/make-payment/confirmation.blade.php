...
<a class="btn btn-outline-brand" href="{{route('portal.dashboard')}}">Return to dashboard</a>
@if($intent->status==='announced')<form method="post" action="{{route('portal.make-payment.cancel',$intent)}}">@csrf @method('delete')<button class="btn btn-outline-danger" type="submit">Cancel notification</button></form>@endif
</div>
</div>
</div></section>
@endsection
