...
@forelse($batches as $batch)<tr><td><a href={{route('admin.property-tax-batches.show',$batch)}}>{{$batch->tax_year}} · {{$batch->label}}</a>@if(filled($batch->property_county))<small class="d-block text-muted">County: {{$batch->property_county}}</small>@endif</td><td>{{$batch->issue_date->format('M j, Y')}} / {{$batch->due_date->format('M j, Y')}}</td><td>{{$batch->status==='not_issued'&&$batch->existing_invoice_count?'Not issued — existing invoices detected':str($batch->status)->replace('_',' ')->title()}}</td><td>{{$batch->rows_count}}</td><td>{{$batch->updated_at->format('M j, Y g:i A')}}</td></tr>
