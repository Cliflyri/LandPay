...
@php($plan=$row->paymentPlan)
@php($membership=$plan?->memberships?->first(fn($m)=>$m->receives_invoices&&$m->role==='primary')??$plan?->memberships?->firstWhere('receives_invoices',true))
...
<tr><td>@if(str_starts_with((string)$rowLabel,'Matched'))<span class="badge rounded-pill" style="background-color:#d1e7dd;color:#0f5132">Matched</span>{{substr((string)$rowLabel,7)}}@else{{$rowLabel}}@endif
