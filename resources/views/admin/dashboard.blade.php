...
<section class="admin-section dashboard-section">
    ...
    <div class="admin-heading dashboard-heading">
        <div>
            ...
            <p class="mb-0">Review account balances, billing status, and automated reminder activity.</p>
        </div>
        ...
        <div class="dashboard-actions">
            ...
            <form method="POST" action="{{ route('admin.logout') }}">
                @csrf
                <button class="btn btn-outline-brand">Sign out</button>
            </form>
        </div>
    </div>
    ...
    <div class="mb-3">
        ...
        <div class="dashboard-summary mb-1" style="margin-top: 0 !important;" aria-label="Portfolio summary">
                ...
                @forelse($plans as $row)
                    ...
                    <tr class="dashboard-plan-row {{ $row['ready_to_close'] ? 'ready-to-close' : '' }}">
                        ...
                        <td class="dashboard-actions-menu">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-start">
                                    <li>
                                        <a class="dropdown-item" href="{{ route('admin.plans.show',$row['plan']) }}">
                                            View plan
                                        </a>
                                    </li>

                                    @if($row['balance_invoice'])
                                        <li>
                                            <a class="dropdown-item" href="{{ route('admin.invoices.show',$row['balance_invoice']) }}">
                                                View invoice
                                            </a>
                                        </li>
                                    @endif
                                    ...
                                    <li>
                                        <a class="dropdown-item" href="{{ route('admin.plans.payments.create',$row['plan']) }}">
                                            Enter payment
                                        </a>
                                    </li>

                                    @if($row['primary_client']?->portalAccount?->enabled)
                                        <li>
                                            <form method="post" action="{{ route('admin.portal-access.store',$row['primary_client']) }}">
                                                @csrf
                                    ...
                                    @else
                                        <li>
                                            <span class="dropdown-item disabled" aria-disabled="true">Portal not active</span>
                                        </li>
                                    @endif

                                    <li>
                                        <a class="dropdown-item" href="{{ route('admin.plans.invoices.create',$row['plan']) }}">
                                            Review next invoice
                                        </a>
                                    </li>
                                    ...
                                    @if(in_array($row['plan']->status, ['active','paused'], true))
                                        <li>
                                            <a class="dropdown-item" href="{{ route('admin.plans.show',$row['plan']) }}#pause-plan-controls">
                                                {{ $row['plan']->status === 'paused' ? 'Resume plan' : 'Pause plan' }}
                                            </a>
                                        </li>
                                    @endif

                                    @if($row['balance_invoice'] && $row['email'])
                                        <li><hr class="dropdown-divider"></li>
                                        <li>
                                            <form
                                                method="post"
                                                action="{{ route('admin.invoices.reminders.store',$row['balance_invoice']) }}"
                                                onsubmit="return confirm('Send a payment reminder to {{ $row['email'] }}?');"
                                            >
                                                @csrf
                                                <button class="dropdown-item" type="submit">Send reminder</button>
                                            </form>
                                        </li>
                                    @endif
                                </ul>
                        ...
                        <td class="text-nowrap">
                            ...
                            @if($row['co_client_count'] > 0)
                                <span
                                    class="co-client-count"
                                    title="{{ $row['co_client_count'] }} co-client(s)"
                                >
                                    +{{ $row['co_client_count'] }}
                                </span>
                            @endif
    ...
    >
        <span class="dashboard-status status-ready-to-close">
            &#10003; Ready to close
                        ...
                        <td
                            class="money-cell dashboard-monthly-column text-center"
                            title="Monthly payment (principal portion)"
                        >
                        ...
                        <td class="money-cell">
                            ...
                            <span class="text-muted">payoff</span>
                        ...
                        <td class="current-balance-cell {{ $row['current_balance_due'] > 0 ? 'balance-due' : '' }}">
                            <strong class="current-balance-total">
                                {{ \App\Support\Money::format($row['current_balance_due']) }}
                            </strong>
                            ...
                            @forelse($row['current_balance_items'] as $item)
                                ...
                                @if($item['invoice'])
                                    <a
                                        class="current-balance-line {{ $isOverdue ? 'is-overdue' : '' }} {{ $isHidden ? 'd-none' : '' }}"
                                        @if($isHidden)
                                            data-current-balance-hidden="balance-items-{{ $row['plan']->id }}"
                                        @endif
                                        href="{{ route('admin.invoices.show',$item['invoice']) }}"
                                        aria-label="{{ \App\Support\Money::format($item['amount']) }} due {{ $dueDate->format('F j, Y') }} on invoice {{ $item['label'] }}"
                                    >
                                        {{ \App\Support\Money::format($item['amount']) }} due {{ $dueLabel }}
                                    </a>
                                @else
                                    <span
                                        class="current-balance-line {{ $isOverdue ? 'is-overdue' : '' }} {{ $isHidden ? 'd-none' : '' }}"
                                        @if($isHidden)
                                            data-current-balance-hidden="balance-items-{{ $row['plan']->id }}"
                                        @endif
                                        title="First payment due {{ $dueDate->format('F j, Y') }}"
                                    >
                                        {{ \App\Support\Money::format($item['amount']) }} due {{ $dueLabel }}
                                        <span class="visually-hidden">(first payment)</span>
                                    </span>
                                @endif
                            ...
                            @if($row['current_balance_items']->count() > 3)
                                ...
                                <button
                                    class="current-balance-more"
                                    type="button"
                                    data-current-balance-toggle="balance-items-{{ $row['plan']->id }}"
                                    data-collapsed-label="+ {{ $hiddenCount }} more {{ $hiddenCount===1?'invoice':'invoices' }}"
                        ...
                        <td>
                            <span class="muted-value">
                                {{ $row['reminder_last_sent'] ?? 'Never' }}
                            </span>
                        </td>

                        <td>
                            <span class="muted-value">
                                {{ $row['reminder_next_send'] ?? 'Not scheduled' }}
                            </span>
                        </td>
                        ...
                        <td>
                            @if($row['email'])
                                <a class="dashboard-email" href="mailto:{{ $row['email'] }}">
                                    {{ $row['email'] }}
                                </a>
                            @else
                                <span class="muted-value">No email</span>
                            @endif
                        </td>
