...
<div class="admin-heading d-flex flex-wrap justify-content-between align-items-end gap-3">
    ...
    <div class="d-flex flex-wrap gap-2">
        ...
        @if($invoice->status !== \App\Enums\InvoiceStatus::Voided && $balance>0 && $smsEligible)<form method="post" action="{{route('admin.invoices.sms-reminder.store',$invoice)}}" onsubmit="return confirm('Send an SMS reminder now?');">@csrf<button class="btn btn-outline-brand text-nowrap" type="submit">Send SMS reminder</button></form>@endif
        ...
        @if($balance>0 && in_array($invoice->status,[\App\Enums\InvoiceStatus::Issued,\App\Enums\InvoiceStatus::PartiallyPaid],true))<button class="btn btn-brand text-nowrap" type="button" data-bs-toggle="modal" data-bs-target="#pay-invoice-full-{{$invoice->id}}-invoice">Pay invoice in full</button>@endif
        <a class="btn btn-outline-brand text-nowrap" href="{{route('admin.plans.payments.create',$invoice->paymentPlan)}}">Enter payment</a>
