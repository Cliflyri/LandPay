...
<p class="mb-0">
    ...
    APN / Plan # {{ $plan->plan_number }}

    <span aria-hidden="true">&middot;</span>

    <span class="fw-bold fs-5">
        Contract balance: {{ \App\Support\Money::format($contractBalance) }}
    </span>
</p>
...
<p class="alert alert-light border mb-3">
    <strong>Plan Payment:</strong> {{ \App\Support\Money::format((int)$plan->customary_monthly_payment) }}, <strong>Fee:</strong> {{ \App\Support\Money::format($serviceFee) }}. Fees are not automatically added to a created invoice.</p>
