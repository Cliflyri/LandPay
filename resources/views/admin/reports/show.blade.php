...
@section('content')
@php
$defs=['payments'=>['Payment report','Payments received and how funds were applied.'],'receivables'=>['Accounts receivable','Open invoice balances organized by age.'],'contracts'=>['Contract balances','Principal, invoice, and credit positions by plan.'],'fees'=>['Fees report','Non-principal fees assessed, waived, collected, and outstanding.'],'client-portals'=>['Client portal status','Invitations, clients without active access, and active portals.']];
[$title,$description]=$defs[$report];$query=request()->except('page');
if(in_array($report,['contracts','client-portals'])){
...
@if($report==='receivables' && $amount>0)<a class="report-summary-card report-summary-link {{$cardActive?'active':''}}" href="{{route('admin.reports.show',array_merge(['report'=>$report],$cardQuery,$cardActive?[]:['aging'=>$label]))}}"><span>{{$label}}</span><strong>{{$report==='client-portals'?$amount:\App\Support\Money::format($amount)}}</strong></a>
@else<div class="report-summary-card"><span>{{$label}}</span><strong>{{$report==='client-portals'?$amount:\App\Support\Money::format($amount)}}</strong></div>@endif
@endforeach</div>
@endif
