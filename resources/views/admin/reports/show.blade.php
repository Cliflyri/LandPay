...
@endforeach</div>
@endif
@if($report==='client-sms')<nav class="nav nav-tabs settings-tabs mt-4"><a class="nav-link {{$filters['sms_tab']==='clients'?'active':''}}" href="{{route('admin.reports.show',array_merge(['report'=>$report,'tab'=>'clients'],request()->only('search')))}}">Clients</a><a class="nav-link {{$filters['sms_tab']==='log'?'active':''}}" href="{{route('admin.reports.show',array_merge(['report'=>$report,'tab'=>'log'],request()->only('search')))}}">SMS Log</a></nav>@endif
...
<thead><tr><th>Client / plan</th><th>Invoice</th><th>Dated</th><th>Due</th><th class="text-end">Original</th><th class="text-end">Paid / credited</th><th class="text-end">Balance</th><th class="text-end">Days overdue</th><th>Aging</th></tr></thead><tbody>
...
@elseif($report==='client-sms' && $filters['sms_tab']==='log')
<thead><tr><th>Date</th><th>Client</th><th>Phone</th><th>Type</th><th>Invoice</th><th>Status</th><th>Sender</th><th>Message</th></tr></thead><tbody>
@forelse($rows as $row) @php($d=$row['delivery'])<tr><td class="text-nowrap">{{$d->created_at->format('M j, Y g:i A')}}</td><td>@if($row['client'])<a href="{{route('admin.clients.show',$row['client'])}}">{{$row['client']->organization_name ?: trim($row['client']->first_name.' '.$row['client']->last_name)}}</a>@else  @endif</td><td>{{$d->recipient_phone}}</td><td>{{str($d->message_type)->replace('_',' ')->title()}}</td><td>@if($row['invoice'])<a href="{{route('admin.invoices.show',$row['invoice'])}}">{{$row['invoice']->invoice_number}}</a>@else  @endif</td><td><span class="badge {{$d->status==='failed'?'text-bg-danger':($d->status==='delivered'?'text-bg-success':'text-bg-secondary')}}">{{str($d->status)->title()}}</span>@if($d->failure_message)<br><small class="text-danger">{{$d->failure_message}}</small>@endif</td><td>{{$row['sender']?->name ?: 'System'}}</td><td><span title="{{$d->message_snapshot}}">{{str($d->message_snapshot)->limit(90)}}</span></td></tr>@empty<tr><td colspan="8" class="report-empty">No SMS deliveries match this search.</td></tr>@endforelse
