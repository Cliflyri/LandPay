...
@if($plan->status === 'draft')
<div class="col-md-4"><label class="form-label">First recurring invoice date</label><input type="date" class="form-control" name="first_scheduled_invoice_date" value="{{ old('first_scheduled_invoice_date',$plan->first_scheduled_invoice_date?->format('Y-m-d')) }}" required><div class="form-text">Sets the recurring invoice day when the plan is activated.</div></div>
@else
