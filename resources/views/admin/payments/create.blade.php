...
@if($input['client_payment_intent_id'] ?? null)<input type="hidden" name="client_payment_intent_id" value="{{$input['client_payment_intent_id']}}"><input type="hidden" name="client_note" value="{{$input['client_note'] ?? ''}}"><input type="hidden" name="invoice_id" value="{{$input['invoice_id'] ?? ''}}"><div class="alert alert-info"><strong>Client payment notification.</strong> Verify receipt before posting.<br><strong>Intended application:</strong> {{filled($input['invoice_id'] ?? null) ? 'Invoice '.($input['invoice_label'] ?? $input['invoice_id']) : 'All open invoices, oldest due first'}}.@if(filled($input['overpayment_disposition'] ?? null))<br><strong>Client overpayment instruction:</strong> {{($input['overpayment_disposition'] ?? null) === 'next_invoice_credit' ? 'Keep extra as account credit.' : 'Apply extra to principal.'}}@endif @if(filled($input['client_note'] ?? null))<br><strong>Client note:</strong> {{$input['client_note']}}@endif</div>@endif
...
<div class="row g-3 mt-1">
    ...
    <div class="col-md-6"><label class="form-label" for="service_fee_amount">Allocate service fee from this payment</label><div class="input-group"><span class="input-group-text">$</span><input class="form-control" id="service_fee_amount" name="service_fee_amount" inputmode="decimal" value="{{ old('service_fee_amount', $input['service_fee_amount'] ?? $defaultServiceFee) }}"></div><div class="form-text">Deducted before the remainder is applied to invoices.</div></div>
...
@foreach($preview['allocations'] as $allocation)@php($isShort=($allocation['due_amount'] ?? $allocation['amount']) > $allocation['amount'])<li><div><strong>{{ $allocation['label'] }}@if($isShort) ({{\App\Support\Money::format($allocation['due_amount'])}} due)@endif</strong>@if($allocation['type'] === \App\Enums\PaymentAllocationType::ServiceFee)<small>Not Invoiced, allocated by this payment screen</small>@endif @if($allocation['invoice_number'] ?? null)<small>@if($allocation['invoice_id'] ?? null)<a href="{{route('admin.invoices.show',$allocation['invoice_id'])}}">Invoice {{ $allocation['invoice_number'] }}</a>@else Invoice {{ $allocation['invoice_number'] }}@endif</small>@endif</div><span @class(['payment-allocation-shortfall'=>$isShort])>{{\App\Support\Money::format($allocation['amount'])}}</span></li>@endforeach
...
<dl class="payment-preview-summary"><div><dt>Applied to invoices</dt><dd>{{\App\Support\Money::format($preview['invoice_amount'])}}</dd></div><div><dt>Applied to principal</dt><dd>{{\App\Support\Money::format($preview['principal_amount'])}}</dd></div><div><dt>Future credit</dt><dd>{{\App\Support\Money::format($preview['credit_amount'])}}</dd></div><div class="payment-outstanding-due"><dt>Outstanding due after payment</dt><dd>{{\App\Support\Money::format(max(0,$currentlyPayable-$preview['invoice_amount']))}}</dd></div></dl>
@endif
</div>
</div>
</div>
