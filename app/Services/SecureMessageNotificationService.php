...
class SecureMessageNotificationService
{
    public function send(SecureMessageThread $thread, bool $reminder = false): bool
    {
        ...
        try {
            Mail::to(strtolower(trim($email)))->send(
                new SecureMessageNotificationMail(route('portal.messages.show', $thread))
            );
            $thread->update([
                'notification_last_sent_at' => now(),
                'notification_status' => 'sent',
                'reminder_count' => $thread->reminder_count + ($reminder ? 1 : 0),
            ]);
            return true;
        } catch (Throwable) {
